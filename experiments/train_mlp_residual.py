...
class MLP(vg.Module):
    ...
    def __init__(self, input_size, output_size, hidden_mult=4):
        ...
        self.residual_layer = vg.Linear(input_size, hidden_size)
    ...
    def forward(self, input):
        ...
        hidden = vg.sigmoid(self.layer1(input))
        
        # Residual connection
        # Apply a linear transformation to input for matching dimensions, if necessary
        residual = self.residual_layer(input)
        # Adding the residual connection

        hidden = hidden + residual

        # Applying non-linearity again (optional, based on design choices)
        hidden = vg.sigmoid(hidden)
        
        # Second layer with softmax activation
        output = vg.logsoftmax(self.layer2(hidden))
        
        return output

    def parameters(self):
        return self.layer1.parameters() + self.layer2.parameters() + self.residual_layer.parameters()
